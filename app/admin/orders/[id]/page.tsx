  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: orderData, error: orderError } = await supabase
        .from("orders")
        .select(
          "id, status, total_amount, shipping_cost, vat_amount, created_at, shipping_method, payment_provider, payment_reference, shipping_address, billing_address"
        )
        .eq("id", id)
      ...
      const { data: itemsData } = await supabase
        .from("order_items")
        .select(
          "id, product_id, quantity, unit_price, products(name)"
        )
        .eq("order_id", id);
